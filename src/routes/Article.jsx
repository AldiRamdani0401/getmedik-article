...
function renderArticle() {
  return article_datas.map((data, index) => {
    return (
      <div class="w-full md:w-1/2 lg:w-1/4 border" key={index} onClick={() => S_Article.showDetail(data.article_id)}>
        <img src={articleBanner} alt="article" />
        <div class="flex flex-col gap-8 px-2 py-3">
          {/* Badge */}
          <div class="flex flex-col gap-2 w-full">
            ...
            <h1 class="font-bold text-lg">{data.title_article}</h1>
          </div>
          {/* Published At */}
          <p class="text-xs font-medium">{H_Article_Format.date(data.published)}</p>
        </div>
      </div>
    );
  });
  ...
  return (
    <div className="flex flex-col gap-8 px-8 py-1 lg:px-20">
        ...
        <div class="flex flex-col justify-between p-5 w-full lg:w-1/2 inset-0 z-20 absolute">
          ...
          <div class="flex flex-col justify-around text-white py-2 gap-3">
            ...
            <p class="text-2xl line-clamp-2 font-bold">
              ...
              dignissimos nobis, eum dolore ipsam iste a. Inventore fugiat ipsa
              et. et consectetur adipisicing elit. Officiis excepturi odit
              veniam quod veritatis expedita
            </p>
            <p class="text-lg line-clamp-2">
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Officiis
              excepturi odit veniam quod veritatis expedita laborum dignissimos
              voluptate ad, dolorem assumenda laboriosam reprehenderit doloribus
              aspernatur voluptatem! Dolor corrupti praesentium molestias
              officiis necessitatibus iste neque eaque adipisci dolorem
              consectetur, fugit, aperiam cumque eius. Velit ducimus, aperiam
              eos sint alias vitae laboriosam.
            </p>
          </div>
        </div>
      </div>
      <div class="flex flex-col gap-5 border">
        ...
        <p>
          Panduan Dan Informasi Kesehatan Untuk Hidup Lebih Sehat Dengan Artikel
          GetMedik
        </p>
      </div>
